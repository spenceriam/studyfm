...
function Pomodoro({ focusMin = 25, shortMin = 5, longMin = 15, onHide, onDragStart }) {
  ...
  return (
    ...
    >
      ...
      <div className="cj-pom__tabs" role="tablist">
        {[
      ...
      <div className="cj-pom__time">
        <span>{mm}</span><span className="cj-pom__colon">:</span><span>{ss}</span>
      </div>
      ...
      <div className="cj-pom__round">
        <span className="cj-pom__round-label">ROUND</span>
        <span className="cj-pom__round-val">#{String(round).padStart(2, '0')}</span>
      </div>
...
function TaskList() {
  ...
  const [tasks, setTasks] = React.useState(() => {
    ...
    return [
      { id: 'demo-1', text: 'Ship landing hero', done: false },
      { id: 'demo-2', text: 'Wire audio stream', done: false },
    ];
  });
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      while (sib) { belowList += sib.offsetHeight; sib = sib.nextElementSibling; }
    ...
    if (wrap) {
      ...
      mo.observe(wrap, { attributes: true, attributeFilter: ['style', 'data-visible'] });
    }
    if (panel) {
  ...
  const addTask = () => {
    ...
    setTasks(ts => [...ts, { id: `t-${Date.now()}-${Math.random().toString(36).slice(2,6)}`, text, done: false }]);
    setDraft('');
    setAdding(false);
  };
  ...
  return (
    <div className="cj-tasks">
      <div className="cj-tasks__hd">
        <span className="cj-tasks__label">TASKS</span>
        <span className="cj-tasks__count">
          {tasks.length === 0 ? 'NONE' : `${doneCount}/${tasks.length}`}
        </span>
      </div>

      {tasks.length > 0 && (
        <ul ref={listRef} className="cj-tasks__list" role="list">
          {tasks.map(t => {
            const isEditing = editingId === t.id;
            return (
            <li key={t.id} className={`cj-task ${t.done ? 'is-done' : ''} ${isEditing ? 'is-editing' : ''}`}>
              <button
              ...
              {isEditing ? (
                ...
                >
                  <span className="cj-task__text-inner">{t.text}</span>
                </span>
              )}
              {!isEditing && (
                <div className="cj-task__actions" aria-hidden={!isEditing ? undefined : true}>
                  {t.done ? (
                    <button
                    ...
                    >
                      <svg viewBox="0 0 16 16" width="11" height="11" aria-hidden="true">
                        <path d="M3 8h7a3 3 0 1 1 0 6H7" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M5.5 5.5 3 8l2.5 2.5" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </button>
                  ) : (
                    <button
                    ...
                    >
                      <svg viewBox="0 0 16 16" width="11" height="11" aria-hidden="true">
                        <path d="M10.5 2.5l3 3L5.5 13.5H2.5V10.5L10.5 2.5z" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M9 4l3 3" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
                      </svg>
                    </button>
                  )}
                  <button
      ...
      {adding ? (
        <div className="cj-tasks__addrow">
          <button
        ...
        <button className="cj-tasks__newbtn" onClick={() => setAdding(true)}>
          <span className="cj-tasks__plus">+</span> ADD TASK
        </button>
      )}
    </div>
  );
...
Object.assign(window, { Pomodoro, TaskList, playBeep });
